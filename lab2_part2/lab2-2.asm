...
elif1:
	...
	sll	$t3, $t3, 8			# shift a to left by 8
	#  n = 110 a 10 b 
	li 	$t4, 0x3000			#  load 1100000000000000 in front
	or 	$s2, $t4, $t3			#  add a to n 
	li 	$t4, 0x40			#  load 1000000  
	or 	$t4, $t4, $t2			#  add b to 1000000 
...
elif2:
	...
	andi	$t2, $s1, 0x03F			#  c = low 6 bits of j 0111111
	srl	$t3, $s1, 6			# move j to right by 6
	andi	$t3, $t3, 0x03F			#  b = next 6 bits of j 111111
	sll 	$t4, $t4, 8			# move b to left by 8	
	srl	$t4, $s1, 12			# shift j right by 12 to get next 4 bits 
	andi 	$t4, $t4, 0xF			#  a = next 4 bits of j 1111
	sll 	$t4, $t4, 16			# shift a left by 16
	#  n = 1110 a 10 b 10 c
	li	$t5, 0x1E0000			# load 111000000000000000000000 to front
	or 	$s2, $t5, $t4			# add a to n
	li 	$t5, 0x2000			# load 10000000000000 
	or	$s2, $s2, $t5			# add 10000000000000  to n
	or	$s2, $s2, $t3			# add b to n
	li 	$t5, 0x40			# load 1000000
...
elif3:
	...
	sll 	$t3, $t3, 8			# move c left by 8
	srl	$t4, $s1, 12			# shift j right by 12
	andi	$t4, $t4, 0x03F			#  b = next 6 bits of j 111111
	sll	$t4, $t4, 16			# move b left by 16
	srl	$t5, $s1, 18			# shift j right by 18 
	andi 	$t5, $t5, 0x7			#  a = next 3 bits of j 111
	sll 	$t5, $t5, 24			# move a left by 24
	#  n = 11110 a 10 b 10 c 10 d
	li 	$t6, 0xF000000			# load 1111000000000000000000000000 to front of n
	or	$s2, $t6, $t5			# add (1111000000000000000000000000 or a) to n 
	li 	$t6, 0x200000			#  load 100000000000000000000000 
	or	$t6, $t6, $t4			# add (100000000000000000000000 or b) to 100000000000000000000000
	or	$s2, $s2, $t6			# add b to n 
	li 	$t6, 0x2000			# load 1000000000000000 
	or	$t6, $t6, $t3			# add c to 1000000000000000
	or	$s2, $s2, $t6			# add (1000000000000000 or c) to n 
	li	$t6, 0x40			# load 10000000
